...
func NewCA(baseDir, org, name, country, province, locality, orgUnit, streetAddress, postalCode string, algo string) (*CA, error) {
	...
	if err == nil {
		...
		priv, _, err := csp.GeneratePrivateKey(baseDir, algo)
		response = err
		if err == nil {
...
// LoadCertificate load a ecdsa or rsa cert from a file in cert path
